...
function getTranscriptionPrompt() {
  return `
    ...
    2. Provide an Executive Summary (under 250 words) highlight the key themes, main arguments, and conclusions of the video.
    3. Determine a suitable title for this presentation or discussion.
...
app.post("/api/transcribe-local", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    const summaryMatch = text.match(/SUMMARY:\s*([\s\S]*?)(?=TRANSCRIPT:|$)/i);
    const transcriptMatch = text.match(/TRANSCRIPT:\s*([\s\S]*)/i);

    res.json({
      title: titleMatch ? titleMatch[1].trim() : req.file.originalname,
...
app.post("/api/transcribe-drive", async (req, res) => {
  ...
  try {
    ...
    res.json({
      title: titleMatch ? titleMatch[1].trim() : (fileName || "Google Drive File"),
      summary: summaryMatch ? summaryMatch[1].trim() : "Summary not specified.",
      transcript: transcriptMatch ? transcriptMatch[1].trim() : text
    });
...
// 3. Endpoint: URL Transcription (Web-grounding URL option)
app.post("/api/transcribe-url", async (req, res) => {
  ...
  try {
    const prompt = `
      ...
      3. Identify the main title or topic.
    ...
    res.json({
      title: titleMatch ? titleMatch[1].trim() : "Video Transcription",
